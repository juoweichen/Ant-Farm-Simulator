...
Your ant colony must move from one room to another. How do you do it in the shortest time possible? This project will get you acquainted with graph traversal algorithms: your program will have to intelligently select paths and precise movements used by the ants.
 
## Project Objectives
The goal of this project is to find the quickest way to get n ants across the farm. The quickest way means the solution with the least number of lines, respecting the output format requested below.<br/>
...
## Traversal rules
1. All ants must start from 'Start' room, their destination is 'End' room
...
## Getting Started
The first step is to create an executable file by Makefile.
```
make
```
...
Alternatively, run quick_run_subjects.sh to test all the farms.
```
sh quick_run_subjects.sh
```
 
Or run quick_run_generator.sh, it's gonna generate some huge map provided by 42 correction sheets.
```
...
turn: 5
```
As a result, it took 5 turns for 5 ants to walk from start to end.
 
# Algorithm
The algorithm would be using [Breath First Search(BFS)](https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/) to looking for the shortest possible path in graph theory. a graph is an abstract data type that consists of nodes and edges. Nodes have been adjacent to other nodes by edges, the same as the farm here.
 
## Storing map to graph
The farm would be stored as a graph in the program before starting the algorithm. I store the graph by using a dictionary. Dictionary is formed by key and value pair, the room's name has been stored as a key in string type, and I create a structure call t_room to store the attribute for each room. The struct defined as follow:
```
...
First, adopting BFS to find the shortest path. For instance, we found the shortest path is B in here:
```
       / [A] - [...] - ... \
       / [B] - [...] - ... \
[Start] - [C] - [...] - ... - [End]
...
// Keep on distributing till all ants have its path...
 
```
## Sending the ants
Last, we send the ants according to our optimize distribution.<br/>
Then...We got it!!!<br/>
**Note:** There's another optimum solution better than this, but this one is easiest for me and it can pass all maps from the generator. <br/>
 
Thank you for bringing those ants home!<br/>
